...
/**
 * Txn Data Two Cc Handler - Reply Flow for Two Cc data.
 */
class TxnDataTwoCcHandler implements HandlerInterface
{
    /**
    ...
    public function setAdditionalInfoForTwoCards($payment, $charges)
    {
        ...
        foreach ($charges as $index => $charge) {
            if (!isset($charge[self::RESPONSE_PAYMENT_METHOD]) || $index > 1) {
                continue;
            }

            $prefix = $prefixes[$index];
            $paymentMethod = $charge[self::RESPONSE_PAYMENT_METHOD];
            ...
            $payment->setAdditionalInformation(
                $prefix . '_' . self::PAYMENT_INFO_CC_INSTALLMENTS,
                $installment
            );
            ...
            $ccTypes[] = $ccType;
            $payment->setAdditionalInformation(
                $prefix . '_' . self::PAYMENT_INFO_CC_TYPE,
                $ccType
            );

            // Card Number
            $ccFirst = $cardData[self::RESPONSE_PM_CARD_FIRST_DIGITS];
            $ccLast = $cardData[self::RESPONSE_PM_CARD_LAST_DIGITS];
            $ccNumber = $ccFirst . 'xxxxxx' . $ccLast;
            $ccLast4s[] = $ccLast;
            $payment->setAdditionalInformation(
                $prefix . '_' . self::PAYMENT_INFO_CC_NUMBER,
                $ccNumber
            );
            ...
            $ccOwners[] = $ccHolderName;
            $payment->setAdditionalInformation(
                $prefix . '_' . self::PAYMENT_INFO_CC_OWNER,
                $ccHolderName
            );

            // Expiration
            $ccExpMonth = $cardData[self::RESPONSE_PM_CARD_EXP_MONTH];
            $ccExpMonths[] = $ccExpMonth;
            $payment->setAdditionalInformation(
                $prefix . '_' . self::PAYMENT_INFO_CC_EXP_MONTH,
                $ccExpMonth
            );

            $ccExpYear = $cardData[self::RESPONSE_PM_CARD_EXP_YEAR];
            $ccExpYears[] = $ccExpYear;
            $payment->setAdditionalInformation(
                $prefix . '_' . self::PAYMENT_INFO_CC_EXP_YEAR,
                $ccExpYear
            );
        }

        // Set concatenated values for payment methods starting with setCc
        $payment->setCcType(implode('|', $ccTypes));
        $payment->setCcLast4(implode('|', $ccLast4s));
        $payment->setCcOwner(implode('|', $ccOwners));
        $payment->setCcExpMonth(implode('|', $ccExpMonths));
        $payment->setCcExpYear(implode('|', $ccExpYears));

        // Set cc number encrypted if available
